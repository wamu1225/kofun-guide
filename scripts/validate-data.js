...
// 4) 内部リンク [/kofun-guide/PATH/] の PATH が実在する記事ID（または about/privacy）であること
import fs from 'fs';
...
const expected = ['basics', 'shapes', 'chambers', 'nintoku', 'scale', 'visiting', 'faq'];
...
for (const { id, block } of blocks) {
  ...
  for (const lk of [...content.matchAll(/\]\(\/kofun-guide\/([a-z0-9-]+)\//g)]) {
    const target = lk[1];
