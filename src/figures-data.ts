...
const FIGURE_DATA: Record<string, { caption: string; inner: string }> = {
  'funkei': {
    caption: '古墳の主な五つの形（模式図）。上から見た輪郭で見分けます。円と方形をつないだ鍵穴形が前方後円墳で、最も格式が高いとされています。帆立貝形は前方部が短い変形にあたります。',
    inner: `<div class="diagram-wrap">${funkeiSvg()}</div>`,
  },
  'sekishitsu': {
    caption: '竪穴式石室と横穴式石室の断面（模式図）。竪穴式は墳頂から縦に穴を掘って密閉し、ひとりを葬ります。横穴式は側面に入り口を開け、玄室と羨道をもち、あとから追葬ができます。',
    inner: `<div class="diagram-wrap">${sekishitsuSvg()}</div>`,
  },
  'sandai-hikaku': {
    caption: '世界三大墳墓の規模くらべ（模式図・おおよその相対比）。ピラミッドは高さで、始皇帝陵は体積で、仁徳天皇陵古墳は平面の長さで最大とされています。日本の巨大古墳は高くそびえるより水平に長く広がります。',
    inner: `<div class="diagram-wrap">${sandaiSvg()}</div>`,
  },
  'fukiishi-haniwa': {
    caption: '築造当時の姿と今の姿の対比（模式図）。築かれた当時は斜面が葺石でおおわれ、段ごとに円筒埴輪の列がめぐっていました。長い年月のあいだに葺石が土や落ち葉に埋もれ、草木が生えて、今は緑の丘のように見えます。',
